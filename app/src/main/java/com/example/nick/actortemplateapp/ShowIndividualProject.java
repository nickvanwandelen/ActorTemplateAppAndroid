...
public class ShowIndividualProject extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //TODO Show only when architect is logged in
    ...
    public boolean onCreateOptionsMenu(Menu menu){
        ...
        showingMenu = menu;
        boolean isAnalist = false;
        String loggedInUserUid = FirebaseAuth.getInstance().getCurrentUser().getUid();


        reference.child("users").child(loggedInUserUid).child("isAnalist").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                isAnalist = dataSnapshot.getValue(boolean.class);
            }
